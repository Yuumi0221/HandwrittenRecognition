...
class Model:
    ...
    def train(self):
        ...
        features_train = features_train.reshape((93240, 28, 28, 1)).astype('float32') / 255
        features_test = features_test.reshape((10360, 28, 28, 1)).astype('float32') / 255
        # 把标签转换为数字([A-Z]->[0-26])
        # labels_train = [ord(x) % 32 - 1 for x in labels_train]
        # labels_test = [ord(x) % 32 - 1 for x in labels_test]
        # one-hot编码形式
        ...
        model.fit(features_train, labels_train, epochs=10, batch_size=64, validation_split=0.1)
        ...
        if self.skip_evaluation:
            ...
            # 在测试集上测试模型性能
            loss, acc = model.evaluate(features_test[:1000], labels_test[:1000])
            print('loss {}, acc {}'.format(loss, acc))

            spinner.succeed(text='Finished evaluation')

        # 保存模型
        model.save("../model/cnn_letter_model.h5")
