...
class Model:
    ...
    def model_fcnn(self):
        ...
        model.add(layers.Dense(units=37, activation='softmax'))
        # 编译
    ...
    def train(self):
        ...
        # 定义模型
        model = self.model_fcnn()

        # 开始训练，传入模型的训练数据形状为(93240, 784) 训练标签为(93240,)
        model.fit(features_train, labels_train, epochs=20, batch_size=128, verbose=2)
        ...
        # 保存模型
        model.save("../model/fcnn_letter_model.h5")
